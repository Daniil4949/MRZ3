...
# Реализация сети Джордана с визуализацией MAPE
# Вариант 11
# Ссылки на источники:
...
INPUT_SIZE = 1
OUTPUT_SIZE = 1
MAX_EPOCHS = 2000  # Увеличено количество эпох
TARGET_LOSS = 1e-8
WINDOW_SIZE = 3
SEED = 42
...
def main():
    """Основная функция для демонстрации использования сети Джордана с визуализацией MAPE."""
    # Генерация арифметической последовательности (1, 4, 7, ..., 25)
    sequence = generate_arithmetic_sequence(start=1, step=3, length=9)  # [1, 4, 7, 10, 13, 16, 19, 22, 25]
    ...
    # Проведение нескольких тренировок для статистической надежности
    all_mapes = []

    for run in range(1, NUM_RUNS + 1):
        logging.info(f"\n=== Тренировка {run} из {NUM_RUNS} ===")
        ...
        all_mapes.append(mape_history)

    # Построение графика зависимости MAPE от эпох для каждой тренировки
    plt.figure(figsize=(10, 6))
    for run in range(NUM_RUNS):
        epochs = range(1, len(all_mapes[run]) + 1)
        plt.plot(epochs, all_mapes[run], label=f'Тренировка {run + 1}')
    plt.xlabel('Эпоха')
    plt.ylabel('MAPE (%)')
    plt.title('Зависимость средней абсолютной процентной ошибки (MAPE) от количества итераций обучения')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.show()

    # Прогнозирование и оценка модели после обучения
